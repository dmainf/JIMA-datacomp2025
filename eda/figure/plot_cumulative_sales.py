...
def process_store(i):
    ...
    final_sales = grouped.groupby('書名', observed=True)['累積売上'].last()
    ...
    if not other_books_df.empty:
        segments = [g[['累積日数', '累積売上']].values for _, g in other_books_df.groupby('書名', observed=True) if len(g) > 1]
        if segments:
    ...
    for book_name in top30_book_names_sorted:
        ...
        if not book_data.empty:
            line, = ax.plot(book_data['累積日数'], book_data['累積売上'], label=str(book_name), alpha=0.8, linewidth=1.5, rasterized=True)
            lines.append(line)
            labels.append(str(book_name))
    ax.set_xlabel('累積日数', fontsize=12)
    ax.set_ylabel('累積売上（円）', fontsize=12)
    ax.set_title(f'書名ごとの累積売上推移（上位30位を強調表示） - 書店{i}', fontsize=14)
    ...
    ticks.sort()
    ax.set_xticks(ticks)
    all_sales_values = grouped['累積売上'].values
