...
def process_store(i):
    ...
    final_sales = grouped.groupby('書名', observed=True)['売上'].sum().sort_values(ascending=False)
    ...
    if len(other_books) > 0:
        ...
        for book_name, book_data in other_books_grouped:
            ax.plot(book_data['累積日数'].values, book_data['売上'].values,
    ...
    title = f'書名ごとの日次売上推移（上位10位を強調表示） - 書店{i}'
    ax.set_title(title, fontsize=14)
    ax.grid(True, alpha=0.3)
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10)
    plt.tight_layout()

    output_file = f'daily_sales_store_{i}.png'
