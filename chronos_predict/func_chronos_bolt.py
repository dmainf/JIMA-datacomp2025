...
def evaluate_predictions(samples, forecasts, config, all_predict):
    ...
    for i, sample in enumerate(samples):
        ...
        book_sales_sum = np.sum(np.abs(target)) + 1e-9
        item_wql_10 = 2 * np.sum(loss_10) / book_sales_sum
        item_wql_50 = 2 * np.sum(loss_50) / book_sales_sum
        item_wql_90 = 2 * np.sum(loss_90) / book_sales_sum
        item_results.append({
            "book_name": book_name,
            "MAE": np.mean(ae),
