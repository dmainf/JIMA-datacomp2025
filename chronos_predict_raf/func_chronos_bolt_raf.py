...
class TimeSeriesRetriever:
    ...
    """
    def search(self, query_ctx, k=1, query_date=None):
        if self.index is None: return []
        q_len = len(query_ctx)
        use_len = min(q_len, self.retrieval_length)
        query_slice = query_ctx[-use_len:]
        if use_len < self.retrieval_length:
            query_slice = np.pad(query_slice, (self.retrieval_length - use_len, 0), 'constant')
        normalized, _ = scale_series(query_slice)
        query_vec = normalized.astype('float32').reshape(1, -1)
        search_k = min(k * 5 + 10, self.index.ntotal)
        D, I = self.index.search(query_vec, search_k)
        results = []
        query_ts = pd.Timestamp(query_date) if query_date is not None else None
        for idx in I[0]:
            if idx == -1: continue
            if query_ts is not None and self.timestamps is not None:
                if pd.Timestamp(self.timestamps[idx]) >= query_ts:
                    continue
            results.append(self.vectors_store[idx])
            if len(results) >= k:
                break
        return results
    """

    def search_batch(self, query_batch, query_dates, k=1):
        if self.index is None:
            return [np.array([]) for _ in range(len(query_batch))]
        ...
        for i in range(N):
            ...
            if len(valid_indices) == 0:
                final_results[i] = np.array([])
                continue
            top_indices = valid_indices[:k]
            vecs = self.vectors_store[top_indices]
            final_results[i] = vecs.flatten()

        return final_results

class ChronosBoltFiDDataset(Dataset):
    """
    Dataset that keeps local context and RAF context as separate tensors.
    Uses raw (unscaled) values -- the model's InstanceNorm handles normalization.
    Missing/invalid positions are filled with NaN so the model can mask them out.
    """

    def __init__(self, df, prediction_length, mode="train", split_ratio=0.9,
                 retriever=None, use_raf=True, context_length=512, raf_length=180,
        ...
        retrieved_results = [np.array([])] * len(sample_info)
        if use_raf and retriever and retriever.index is not None and sample_info:
            ...
            retrieved_results = retriever.search_batch(query_matrix, query_dates_arr, k=top_k)

        for idx, info in enumerate(tqdm(sample_info, desc="Building Samples")):
            retrieved = retrieved_results[idx]
            local_ctx = info['local_ctx']

            if use_raf and len(retrieved) > 0:
                raf_ctx = retrieved.astype(np.float32)
                if len(raf_ctx) > raf_length:
                    raf_ctx = raf_ctx[-raf_length:]
                if len(raf_ctx) < raf_length:
                    raf_ctx = np.concatenate([
                        np.full(raf_length - len(raf_ctx), np.nan, dtype=np.float32),
                        raf_ctx
                    ])
            else:
                raf_ctx = np.full(raf_length, np.nan, dtype=np.float32)
            ...
            raf_mask_tensor = ~torch.isnan(raf_context_tensor)
...
def train_model(df, retriever=None):
    ...
    model = ChronosBoltFiDModel.from_pretrained(
        CONFIG["model_name"], device_map="auto", torch_dtype=dtype
    )
...
def save_results(samples, forecasts, decile_books, all_predict):
    ...
    for i, sample in enumerate(tqdm(samples, desc="Saving Plots")):
        ...
        if not all_predict and book_name not in decile_books:
            continue
        #pred = np.expm1(forecasts[i].numpy())
        #target = np.expm1(sample["target"])
        #history = np.expm1(sample.get("query", np.array([])))
...
def evaluate_predictions(samples, forecasts, all_predict):
    ...
    #pred_array = np.expm1(np.stack([f.numpy() for f in forecasts]))
