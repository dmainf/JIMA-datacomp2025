...
def evaluate_predictions(samples, forecasts, all_predict):
    ...
    # Add
    if all_predict:
        item_pred_total_sales = np.sum(q50, axis=1)

        pred_cols = [f"Prediction_{i}" for i in range(1, pred_len + 1)]
        pred_df = pd.DataFrame(q50, columns=pred_cols)

        pred_total_series = pd.Series(item_pred_total_sales, name="Prediction_Total_Sales")
    ...
    # Add
    if all_predict:
        df_res.insert(len(df_res.columns), "Prediction_Total_Sales", pred_total_series.values)
