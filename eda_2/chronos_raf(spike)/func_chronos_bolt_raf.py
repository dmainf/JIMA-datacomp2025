...
def evaluate_predictions(samples, forecasts, all_predict):
    ...
    df_res.to_csv(f"{CONFIG['output_dir']}/{fname}", index=False, encoding="utf-8-sig", quoting=csv.QUOTE_ALL)
...
def evaluate_spike_predictions(
    ...
    fname = "evaluation_all" if all_predict else "evaluation_decile"
    out_csv = f"{CONFIG['output_dir']}/{fname}_{csv_suffix}.csv"
    df_spa.to_csv(out_csv, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_ALL)
